"""
AsciiLionSprite — visual representation of a Lion on the zoo map.

Uses callbacks instead of PyQt signals because QGraphicsPixmapItem
is not a QObject in Qt6.

Tests:
    - test_pixmap_cache_returns_same_object: Call _render_lion_pixmap("#e8a838")
      twice; verify the same QPixmap object is returned.
    - test_dead_lion_renders_in_red: Call update_state(x, y, is_dead=True);
...
def _render_lion_pixmap(color: str) -> QPixmap:
    """Render ASCII_LION text to a cached QPixmap.

    Tests:
        - test_render_returns_pixmap: Verify a QPixmap is returned.
        - test_cache_is_populated: Verify key exists in _LION_CACHE after call.
    """
    if color in _LION_CACHE:
        return _LION_CACHE[color]

    lines = ASCII_LION.split("\n")
    if not lines:
        return QPixmap()
    ...
    _LION_CACHE[color] = pixmap
    return pixmap


class AsciiLionSprite(QGraphicsPixmapItem):
    """ASCII-art lion on the zoo map.

    Renders a cached QPixmap from ASCII_LION text. Golden (#e8a838)
    when alive, red (#f85149) when dead. 100×59 px final size.

    Tests:
        - test_lion_is_golden_by_default: Create sprite; verify pixmap is
          not the dead (red) variant.
        - test_dead_lion_switches_pixmap: Call update_state(x, y, True);
    ...
    def update_state(self, x: float, y: float, is_dead: bool) -> None:
        ...
        Args:
            ...
            is_dead: If True, render red (dead) pixmap; False restores golden.
        ...
        if is_dead and not self._is_dead:
            self._pixmap = _render_lion_pixmap(C_RED)
            self.setPixmap(self._pixmap)
            self._is_dead = True
            ...
            self.setPixmap(self._pixmap)
            self._is_dead = False
            self.setToolTip(f"{self._name} · Löwe")
    ...
    def animal_id(self) -> str:
        ...
        Tests:
            - test_returns_correct_id: Create with id "a_07"; verify
              animal_id property returns "a_07".
        """
    ...
    def hoverEnterEvent(self, event: object) -> None:
        """Fire hover callback with this lion's animal_id.
